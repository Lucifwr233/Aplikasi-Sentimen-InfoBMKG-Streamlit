...
# --- FUNGSI UNTUK VISUALISASI DAN METRIK ---
...
def plot_sentiment_barchart(data, column, title, ax):
    ...
    ax.set_title(title, fontsize=14); ax.set_ylabel('Jumlah'); ax.tick_params(axis='x', rotation=0)
    for i, v in enumerate(counts): ax.text(i, v + (counts.max()*0.01), str(v), ha='center')
...
def plot_wordcloud(df, sentiment, ax):
    ...
    if filtered_df.empty:
        ax.text(0.5, 0.5, f"Tidak ada data '{sentiment}'", ha='center', va='center'); return
    all_words = [word for tokens in filtered_df['Komentar_Tokenized'] for word in tokens]
    if not all_words:
        ax.text(0.5, 0.5, f"Tidak ada kata '{sentiment}'", ha='center', va='center'); return
...
def plot_confusion_matrix(y_true, y_pred, title, ax):
    ...
    ax.set_title(title); ax.set_ylabel('Label Aktual'); ax.set_xlabel('Label Prediksi')
...
def show_dataset_page():
    ...
    st.markdown("Berikut adalah dataset asli yang digunakan dalam proyek ini.")
    df_asli = load_data('Dataset/1 Dataset Asli.csv')
    if df_asli is not None:
        st.dataframe(df_asli)

def show_visualization_page():
    st.title("Visualisasi dan Evaluasi Data")
    st.markdown("Dasbor ini menampilkan visualisasi distribusi sentimen, kata-kata yang sering muncul, dan evaluasi kinerja model.")
...
menu = st.sidebar.radio(
    "Pilih Halaman:",
    ("Analisis Teks", "Tampilkan Dataset", "Visualisasi & Evaluasi")
)

st.sidebar.info("Aplikasi ini dibuat untuk mendemonstrasikan perbandingan model klasifikasi teks untuk ulasan aplikasi Info BMKG.")

if menu == "Analisis Teks":
    show_prediction_page()
elif menu == "Tampilkan Dataset":
    show_dataset_page()
elif menu == "Visualisasi & Evaluasi":
